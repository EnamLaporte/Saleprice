...
st.write("""
# Application simlpe pour MaMaison.sn - Prédiction des prix de vente des maisons
Cette application prédit le prix de vente des maisons à partir des caractéristiques entrées !!!""")

st.sidebar.header("Choisissez les caractéristiques de la maison !")
...
def predict_price (input):
    ...
    # Renommage du nom de la colonne 1stFlrSF
    ...
    # Sélection des noms de colonnes avec un pourcentage de valeurs manquantes supérieur à 50%
    missing_features = percent_missing[percent_missing > 30].index

    # Suppriession de ces colonnes du dataframe
    train_data.drop(missing_features, axis=1, inplace=True)
    train_data.drop(['Id','SaleType','Street','LandContour','GarageQual','GarageCond','PavedDrive','SaleType'], axis=1, inplace=True)
    test_data.drop(['Id','SaleType','Street','LandContour','GarageQual','GarageCond','PavedDrive','SaleType'], axis=1, inplace=True)
    ...
    # Remplacement des valeurs manquantes pour les colonnes de type "object"
    ...
    # Calcul du nombre de zéros dans chaque colonne
    zeros = train_data.eq(0).sum()

    # Sélection des colonnes ayant moins de 50% de zéros
    threshold = 0.5 * len(train_data)
    cols_to_keep = zeros[zeros < threshold].index

    # Suppression des colonnes ayant plus de 50% de zéros
    train_data = train_data[cols_to_keep]

    # ajout d'un bouton qui permettra d'afficher le tableau
    show_table = st.expander("Le Jeu de Donnée (train.csv) après néttoyage")

    # ajout du code qui sera exécuté lorsque le bouton sera cliqué
    with show_table:
...
#Affichage de la distribution des valeurs dans la colonne 'SalePrice' du train_data
...
st.subheader("Auteur : Josaphat.H's team")
